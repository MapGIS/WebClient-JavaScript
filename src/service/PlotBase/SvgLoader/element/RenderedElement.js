...
class RenderedElement extends Element {
  ...
  getMapScaleLineWidth() {
    ...
    if (strokeWidthStyleProp.hasValue()) {
      const newLineWidth =
        strokeWidthStyleProp.getPixels() * this.getMapScale();
  ...
  _clone(cloneObject) {
    ...
    if (this._transformMatrix) {
      if (Array.isArray(this._transformMatrix)) {
        cloneObject._transformMatrix = this._transformMatrix.map((s) =>
          s.clone()
        );
      } else {
  ...
  _getSaveBaseAttributes(BaseClass, superAttributesObject) {
    ...
    baseAttributes.baseSVGAttributes = baseSVGAttributes.concat(
      superAttributesObject && superAttributesObject.baseSVGAttributes
        ? superAttributesObject.baseSVGAttributes
        : []
    );

    baseAttributes.extendElementAttributes = extendElementAttributes.concat(
      superAttributesObject && superAttributesObject.extendElementAttributes
        ? superAttributesObject.extendElementAttributes
        : []
    );
