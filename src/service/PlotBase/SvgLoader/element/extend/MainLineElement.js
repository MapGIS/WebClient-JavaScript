...
export default class MainLineElement extends MainElement {
    ...
    deleteArr(mainLine, width, scale, defaultLine, flag = this.flag) {
        ...
        let rateArr = [];
        clip.forEach((s) => {
            const originRate = flag ? s.rate * middleRate + firstRate : 1 - (s.rate * middleRate + firstRate);
            ...
            if (s.type === 'start') {
                const [bzIndex, rate] = this.getRateByPx(lengthArr, lengthIndex, pathRate, s.px * scale, !flag);
                if (flag) {
                    rateArr = rateArr.concat(this._resetRateArr([bzIndex, 0], [lengthIndex, pathRate]));
                } else {
                    rateArr = rateArr.concat(this._resetRateArr([lengthIndex, pathRate], [bzIndex, 1]));
                }
            }

            if (s.type === 'on') {
                const [bzIndex2, rate2] = this.getRateByPx(lengthArr, lengthIndex, pathRate, (s.px / 2) * scale, false);
                const [bzIndex1, rate1] = this.getRateByPx(lengthArr, lengthIndex, pathRate, (s.px / 2) * scale, true);
                if (bzIndex1 !== bzIndex2 || Math.abs(rate2 - rate1) > 10e-8) {
                    rateArr = rateArr.concat(this._resetRateArr([bzIndex2, rate2], [bzIndex1, rate1]));
                }
            }
    ...
    _getRatePathStr(i, arr, mainLine) {
        ...
        let path = '';
        arr.forEach((t) => {
            path += mainLine.getBPathStr(s[0], s[1], s[2], s[3], s[4], s[5], s[6], s[7], t[0], t[1]);
        });
        return path;
    }
    ...
    getCoords() {
        ...
        return pathArr.filter((t) => t.length !== 0);
    }
}
