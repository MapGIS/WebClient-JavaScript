...
export default class PlotBlinkAnimation extends PlotColorAnimation {
  ...
  _applyColorByRate(colorItem, rate) {
    ...
    const blinkColors = this.blinkColors;
    ...
    if (blinkColors.length === 0) {
      ...
      color = this.isBlinkGrad
        ? this._calcColorArrByRate(
            blinkColors.concat([this._calcColorRate(lastColor, 0)]),
            rate
          )
        : this._calcColorArrByRateWithNoGrad(
            blinkColors.concat([this._calcColorRate(lastColor, 0)]),
            rate
          );
    }
    ...
    keys.forEach((s) => {
      if (Object.prototype.toString.call(colorItem[s]) === "[object Object]") {
        ...
        ? color
        : (this.isBlinkGrad
        ? this._calcColorRate(colorItem[s], rate)
        : colorItem[s])
      }
    });
  }
