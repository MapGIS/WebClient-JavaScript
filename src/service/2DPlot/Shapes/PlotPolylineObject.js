...
const _ = require("lodash");
 ...
 const PlotPolylineObject = fabric.util.createClass(PlotObject, {
  ...
  setPnts: function setPnts(coords) {
    this.callSuper("setPnts", coords);
    this.set("dirty", true);
  },
  ...
  _drawPath(ctx, style) {
    ...
    if (
      style.fillStyleType > 0 &&
      defined(style.fillStyle) &&
      style.fillStyle !== "" &&
  ...
  _pointsToPath: function _pointsToPath(ctx, style, coords) {
    ...
    for (let j = 0; j < pnts.length; j += 1) {
      ...
      ctx.beginPath();
      ctx.moveTo(
        tempPnts[0].x + this.m_offsetX,
        tempPnts[0].y + this.m_offsetY
      );
      for (let m = 1; m < tempPntsLen; m += 1) {
        ctx.lineTo(
          tempPnts[m].x + this.m_offsetX,
          tempPnts[m].y + this.m_offsetY
        );
      }
      if (
        GeomUtil.PointEqualFuzzy(startPnt.x, startPnt.y, endPnt.x, endPnt.y)
      ) {
        ctx.closePath();
      }
  ...
  _comparePathStyle: function _comparePathStyle(svgStyle) {
    ...
    // 对于线划部分，需要加入基础宽度
    if (
      (strokeStyle === "none" || strokeStyle === "rgba(0,0,0,0)") &&
      fillStyle &&
      fillStyle !== "none" &&
  ...
  innerInitControls: function innerInitControls() {
    function polyLinePositionHandler(dim, finalMatrix, fabricObject) {
      const x =
        fabricObject.m_coordsPx[this.pointIndex].x + fabricObject.m_offsetX;
      const y =
        fabricObject.m_coordsPx[this.pointIndex].y + fabricObject.m_offsetY;
    ...
    function actionHandler(eventData, transform, x, y) {
      ...
      const mouseLocalPosition = object.toLocalPoint(
    ...
    function anchorWrapper(anchorIndex, fn) {
      ...
      return function (eventData, transform, x, y) {
        ...
        const newX =
          (fabricObject.m_coordsPx[anchorIndex].x + fabricObject.m_offsetX) /
          polygonBaseSize.x;
        const newY =
          (fabricObject.m_coordsPx[anchorIndex].y + fabricObject.m_offsetY) /
          polygonBaseSize.y;
