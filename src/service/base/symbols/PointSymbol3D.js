 ...
 * @classdesc 三维点符号
 * @param {String} [type] 类型，只能是'point-3d'
 ...
 * @param {Array<IconSymbol3DLayer|ObjectSymbol3DLayer|TextSymbol3DLayer>} [symbolLayers] 图层集合，用来可视化要素和制图综合
...
export default class PointSymbol3D extends Symbol3D {
    ...
    fromJSON(json) {
        json = json || {};
        const { type = 'point-3d' } = json;
        const { callout = undefined } = json;
        const { styleOrigin = { name: 'mapgis-style', styleName: 'styleName' } } = json;
        const { symbolLayers, verticalOffset } = json;
...
mapgis.symbols.PointSymbol3D = PointSymbol3D;
