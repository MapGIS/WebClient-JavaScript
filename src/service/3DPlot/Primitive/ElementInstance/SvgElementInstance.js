...
export default class SvgElementInstance {
  ...
  svgToGeomInstances(elem, options, callback) {
    ...
    let sampleElevationTool = new Cesium.SampleElevationTool(window.viewer, this.samplePoints, 'terrain', function (sampleResult) {
      let pathOffsetHeights = that._getOffsetHeights(sampleResult);
      let spanOffsetHeights = that._getOffsetHeights(sampleResult, 'span');
      let instances = [];
      let wallOffsetHeights = [], wallOffsetIndex = 0;
      if (pathOffsetHeights.length !== paths.length) {
        console.error("pathOffsetHeights长度有误！");
      }
      for (let i = 0; i < paths.length; i += 1) {
        if (paths[i].type === 'mainline' || paths[i].type === 'extendline') {
        ...
        let pathTempInst = that.pathElemToGeomInstance(paths[i], options);
        if (!defined(pathTempInst)) continue;
        //针对circle和path，只能更改其生成的三维体的高度
        if (paths[i].type === 'circle' || paths[i].type === 'path') {
      ...
      for (let i = 0; i < spans.length; i += 1) {
        let tempInst = that.spanElemToGeomInstance(spans[i], options);
      ...
      callback(instances, wallOffsetHeights, wallOffsetIndex);
    }, {level: 10});
