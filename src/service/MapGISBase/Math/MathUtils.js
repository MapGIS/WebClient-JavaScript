...
function calculatePolygonGravityCenter(points) {
  var area = 0.0; // 多边形面积
  var gravityX = 0.0; // 重心点X
  var gravityY = 0.0; // 重心点Y

  if (points.length <= 2) {
    throw new Error("点数量错误，无法构成多边形");
  }
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    let x = point[0];
    let y = point[1];
    let nextX = points[(i + 1) % points.length][0];
    let nextY = points[(i + 1) % points.length][1];
    // 2
