...
import { convertBytesToHex,convertHexToBytes } from 'MapGISBase/Util/StringUtil'
...
function convertUtf8ToBytes(text) {
  var result = [],
    i = 0;
  text = encodeURI(text);
  while (i < text.length) {
...
function convertBytesToUtf8(bytes) {
  var result = [],
    i = 0;
...
function pkcs7strip(data) {
  ...
  ArrayUtil.copyArr(data, result,0, length, 0);
  return result;
}
...
class AES {
  ...
  _init() {
    ...
    const tk = byteArrToInt32Arr(this.key);
    ...
    while (t < roundKeyCount) {
      ...
      if (KC != 8) {
        for (let i = 1; i < KC; i += 1) {
          tk[i] ^= tk[i - 1];
        }

        // key expansion for 256-bit keys is "slightly different" (fips-197)
      } else {
        for (let i = 1; i < KC / 2; i += 1) {
          tk[i] ^= tk[i - 1];
        }
        tt = tk[KC / 2 - 1];
  ...
  encrypt(plainText) {
    ...
    let t = byteArrToInt32Arr(plainText);
    for (let i = 0; i < 4; i += 1) {
      t[i] ^= this._Ke[0][i];
    }
  ...
  decrypt(cipherText) {
    ...
    var t = byteArrToInt32Arr(cipherText);
    for (var i = 0; i < 4; i++) {
      t[i] ^= this._Kd[0][i];
    }
...
class CBC extends AES {
  ...
  encrypt(utf8)
  {
    const bytes=pkcs7pad(convertUtf8ToBytes(utf8));
    const cipherText=this._encrypt(bytes);
    return convertBytesToHex(cipherText);
  }

  decrypt(hex)
  {
    const bytes=convertHexToBytes(hex);
    const plainText=this._decrypt(bytes);
    return convertBytesToUtf8(pkcs7strip(plainText));
  }
  ...
  _encrypt(plainText) {
    ...
    for (let i = 0; i < plainText.length; i += 16) {
      ArrayUtil.copyArr(plainText, block, i, i + 16, 0);

      for (let j = 0; j < 16; j += 1) {
        block[j] ^= this._lastCipherBlock[j];
      }

      this._lastCipherBlock = super.encrypt(block);
      ArrayUtil.copyArr(this._lastCipherBlock, ciphertext, 0, 16, i);
    }
  ...
  _decrypt(cipherText) {
    ...
    for (let i = 0; i < cipherText.length; i += 16) {
      ArrayUtil.copyArr(cipherText, block, i, i + 16, 0);
      block = super.decrypt(block);
      for (let j = 0; j < 16; j += 1) {
        plainText[i + j] = block[j] ^ this._lastCipherBlock[j];
      }
      ArrayUtil.copyArr(cipherText, this._lastCipherBlock, i, i + 16, 0);
    }
 ...
 * @returns 
 */
export function aesEncrypt_CBC(key,plainText,iv)
{
  const keyBytes=convertUtf8ToBytes(key);
  let ivBytes;
  if(defined(iv)) ivBytes=convertUtf8ToBytes(iv);
  const cbc= new CBC(keyBytes,ivBytes);
 ...
 * @param {*} iv 
 */
export function aesDecrypt_CBC(key,cipherText,iv)
{
  const keyBytes=convertUtf8ToBytes(key);
  let ivBytes;
  if(defined(iv)) ivBytes=convertUtf8ToBytes(iv);
