...
export var TimeDimension = (L.Layer || L.Class).extend({
    ...
    _generateAvailableTimes: function () {
        if (this.options.times) {
            return TimeDimensionUtil.parseTimesExpression(this.options.times);
        } else if (this.options.timeInterval) {
            var tiArray = TimeDimensionUtil.parseTimeInterval(this.options.timeInterval);
            var period = this.options.period || 'P1D';
            var validTimeRange = this.options.validTimeRange || undefined;
            return TimeDimensionUtil.explodeTimeRange(tiArray[0], tiArray[1], period, validTimeRange);
        } else {
            return [];
        }
    },
    ...
    setAvailableTimes: function (times, mode) {
        ...
        if (mode == 'extremes') {
            var period = this.options.period || 'P1D';
            this._availableTimes = TimeDimensionUtil.explodeTimeRange(new Date(times[0]), new Date(times[times.length - 1]), period);
        } else {
            var parsedTimes = TimeDimensionUtil.parseTimesExpression(times);
            if (this._availableTimes.length === 0) {
                this._availableTimes = parsedTimes;
            } else if (mode == 'intersect') {
                this._availableTimes = TimeDimensionUtil.intersect_arrays(parsedTimes, this._availableTimes);
            } else if (mode == 'union') {
                this._availableTimes = TimeDimensionUtil.union_arrays(parsedTimes, this._availableTimes);
...
let timeDimension = function (options) {
    return new TimeDimension(options);
};

L.zondy.TimeDimension = timeDimension;