...
export function IgsTileProvider(options) {
  ...
  function buildImageResource(imageryProvider, x, y, level, request) {
    ...
    if (imageUrl.indexOf("{row}") < 0
      && imageUrl.indexOf("{col}") < 0
      && imageUrl.indexOf("{level}") < 0) {
        imageUrl += '/' + level + '/' + y + '/' + x
    } else {
      imageUrl = imageUrl.replace("{row}", y);
      imageUrl = imageUrl.replace("{col}", x);
      //修改说明：兼容分辨率为512的瓦片
      //修改人：韩彦生 2018-04-06
      if (
        imageryProvider._tileWidth === 512 ||
        imageryProvider._mapStyle === "mapgis-tianditu"
      ) {
        imageUrl = imageUrl.replace("{level}", level + 1);
      } else {
...
CesiumZondy.Provider.MapGISMapImageProvider = IgsTileProvider;
CesiumZondy.Provider.IgsTileProvider = IgsTileProvider;
