...
import {SmicText} from '../levelRender/SmicText';
import {SmicCircle} from '../levelRender/SmicCircle';
import {SmicBrokenLine} from '../levelRender/SmicBrokenLine';
import {SmicImage} from '../levelRender/SmicImage';
import {SmicPolygon} from '../levelRender/SmicPolygon';
import {SmicRectangle} from '../levelRender/SmicRectangle';
import {SmicSector} from '../levelRender/SmicSector';
 ...
 * 目前支持创建的图形有：<br>
 * 用于统计专题图：<br>
 * 点 - 参数对象 <{@link Zondy.Feature.ShapeParameters.Point}> <br>
 * 线 - 参数对象 <{@link Zondy.Feature.ShapeParameters.Line}> <br>
 * 面 - 参数对象 <{@link Zondy.Feature.ShapeParameters.Polygon}> <br>
 * 矩形 - 参数对象 <{@link Zondy.Feature.ShapeParameters.Rectangle}> <br>
 * 扇形 - 参数对象 <{@link Zondy.Feature.ShapeParameters.Sector}> <br>
 * 标签 - 参数对象 <{@link Zondy.Feature.ShapeParameters.Label}> <br>
 * 图片 - 参数对象 <{@link Zondy.Feature.ShapeParameters.Image}> <br>
 * 用于符号专题图：<br>
 * 圆形 -  参数对象：<{@link Zondy.Feature.ShapeParameters.Cilcle}>
 */
class ShapeFactory {


    /**
     * @function Zondy.Feature.ShapeFactory.prototype.constructor
     * @description 构建图形工厂对象。
     * @param {Object} shapeParameters - 图形参数对象，<{@link Zondy.Feature.ShapeParameters}> 子类对象，可选参数。
     * @returns {Zondy.Feature.ShapeFactory} 返回图形工厂类对象。
     */
    constructor(shapeParameters) {
        /**
         * @member {Object} Zondy.Feature.ShapeParameters.prototype.shapeParameters
         * @description  图形参数对象，<{@link Zondy.Feature.ShapeParameters}> 子类对象。必设参数，默认值 null。
         */
    ...
    /**
     * @function  Zondy.Feature.ShapeParameters.prototype.createShape
     * @description 创建一个图形。具体图形由 shapeParameters 决定。
     * @param {Object} shapeParameters - 图形参数对象，<{@link Zondy.Feature.ShapeParameters}> 子类对象。
    ...
    createShape(shapeParameters) {
        ...
        if (sps instanceof Point) {        // 点
            ...
            style = copyAttributesWithClip(style, sps.style, ['x', 'y']);
            ...
            copyAttributesWithClip(shape, sps, ['x', 'y', 'style', 'highlightStyle']);

            return shape;
        } else if (sps instanceof FeatureLine) {        // 线
            //检查参数 pointList 是否存在
            if (!sps.pointList) {
                return null;
            }
            ...
            style = copyAttributesWithClip(style, sps.style, ['pointList']);

            // 创建图形
            let shape = new SmicBrokenLine();
            shape.style = ShapeFactory.transformStyle(style);
            shape.highlightStyle = ShapeFactory.transformStyle(sps.highlightStyle);
            copyAttributesWithClip(shape, sps, ['pointList', 'style', 'highlightStyle']);

            return shape;
        } else if (sps instanceof FeaturePolygon) {        // 面
            //检查参数 pointList 是否存在
            if (!sps.pointList) {
                return null;
            }
            ...
            return shape;
        } else if (sps instanceof FeatureRectangle) {        // 矩形
            //检查参数 pointList 是否存在
            ...
            style = copyAttributesWithClip(style, sps.style, ['x', 'y', 'width', 'height']);

            //创建图形
            let shape = new SmicRectangle();
            shape.style = ShapeFactory.transformStyle(style);
            shape.highlightStyle = ShapeFactory.transformStyle(sps.highlightStyle);
            copyAttributesWithClip(shape, sps, ['x', 'y', 'width', 'height', 'style', 'highlightStyle']);
            ...
            style = copyAttributesWithClip(style, sps.style, ['x', 'y', 'r', 'startAngle', 'endAngle', 'r0', 'endAngle']);

            //创建图形
            let shape = new SmicSector();
            shape.style = ShapeFactory.transformStyle(style);
            shape.highlightStyle = ShapeFactory.transformStyle(sps.highlightStyle);
            copyAttributesWithClip(shape, sps, ['x', 'y', 'r', 'startAngle', 'endAngle', 'r0', 'endAngle', 'style', 'highlightStyle']);
            ...
            style = copyAttributesWithClip(style, sps.style, ['x', 'y', 'text']);

            //创建图形
            let shape = new SmicText();
            shape.style = ShapeFactory.transformStyle(style);
            shape.highlightStyle = ShapeFactory.transformStyle(sps.highlightStyle);
            copyAttributesWithClip(shape, sps, ['x', 'y', 'text', 'style', 'highlightStyle']);
            ...
            style = copyAttributesWithClip(style, sps.style, ['x', 'y', 'image', 'width', 'height', 'sx', 'sy', 'sWidth', 'sHeight']);

            //创建图形
            let shape = new SmicImage();
            shape.style = ShapeFactory.transformStyle(style);
            shape.highlightStyle = ShapeFactory.transformStyle(sps.highlightStyle);
            copyAttributesWithClip(shape, sps, ['x', 'y', 'image', 'width', 'height', 'style', 'highlightStyle']);

            return shape;
        } else if (sps instanceof FeatureCircle) {       //圆形 用于符号专题图
            //设置stytle
            let style = new Object();
            style["x"] = sps.x;
            style["r"] = sps.r;
            ...
            style = copyAttributesWithClip(style, sps.style, ['x', 'y', 'r']);
            ...
            copyAttributesWithClip(shape, sps, ['x', 'y', 'r', 'style', 'highlightStyle', 'lineWidth', 'text', 'textPosition']);
    ...
    /**
     * @function  Zondy.Feature.ShapeParameters.prototype.transformStyle
    ...
    /**
     * @function  Zondy.Feature.ShapeParameters.prototype.Background
     * @description 创建一个矩形背景框图形对象。
     * @param {Zondy.Feature.ShapeFactory} shapeFactory - 图形工厂对象，必设参数。
     ...
     * @param {Object} setting.backgroundStyle - 背景样式，此样式对象对象可设属性：<Zondy.Feature.ShapeParameters.Rectangle#style>。
    ...
    static Background(shapeFactory, box, setting) {
        ...
        // 背景框图形参数对象
        var bgSP = new FeatureRectangle(box[0], box[3], Math.abs(box[2] - box[0]), Math.abs(box[3] - box[1]));
        ...
        if (sets.backgroundStyle) {
            copyAttributesWithClip(bgSP.style, sets.backgroundStyle);
        }
    ...
    /**
     * @function  Zondy.Feature.ShapeParameters.prototype.GraphAxis
     ...
     * @param {Object} setting.axisStyle - 坐标轴样式，此样式对象对象可设属性：<Zondy.Feature.ShapeParameters.Line#style>。
     ...
     * @param {Object} setting.axisYLabelsStyle - y 轴上的标签组样式，此样式对象对象可设属性：<Zondy.Feature.ShapeParameters.Label#style>。
     ...
     * @param {Object} setting.axisXLabelsStyle - x 轴上的标签组样式，此样式对象对象可设属性：<Zondy.Feature.ShapeParameters.Label#style>。
     ...
     * @param {Object} setting.xReferenceLineStyle - 水平参考线样式，此样式对象对象可设属性：<Zondy.Feature.ShapeParameters.Line#style>。
    ...
    static GraphAxis(shapeFactory, dataViewBox, setting, xShapeInfo) {
            ...
            for (var i = 0; i < axisytick; i++) {
                ...
                if (isAddRefLine) {
                    ...
                    if (sets.xReferenceLineStyle) {
                        copyAttributesWithClip(refLineSP.style, sets.xReferenceLineStyle);
                    }
                    // 生成参考线图形对象
        ...
        if (sets.axisUseArrow) {
            ...
            //x轴箭头
            var xSP = new FeaturePolygon(xArrowPois);
            xSP.style = {fillColor: "#008acd"};
            copyAttributesWithClip(xSP.style, sets.axisStyle);
            arrows.push(shapeFactory.createShape(xSP));

            //y轴箭头
            var ySP = new FeaturePolygon(yArrowPois);
            ySP.style = {fillColor: "#008acd"};
            copyAttributesWithClip(ySP.style, sets.axisStyle);
            arrows.push(shapeFactory.createShape(ySP));

            // z轴箭头 坐标轴箭头是否要使用
            if (sets.axis3DParameter && !isNaN(sets.axis3DParameter) && sets.axis3DParameter >= 15) {
                var zSP = new FeaturePolygon(zArrowPois);
                zSP.style = {fillColor: "#008acd"};
                copyAttributesWithClip(zSP.style, sets.axisStyle);
        ...
        // 坐标轴参数对象
        var axisSP = new FeatureLine(pois);
        // 坐标轴默认style
        axisSP.style = {
            strokeLinecap: "butt",
            strokeLineJoin: "round",
        ...
        // 用户 style
        if (sets.axisStyle) {
            copyAttributesWithClip(axisSP.style, sets.axisStyle);
        }
        // 禁止事件
        ...
        if (sets.axisYLabels && sets.axisYLabels.length && sets.axisYLabels.length > 0) {
            ...
            if (len === 1) {
                ...
                if (sets.axisYLabelsStyle) {
                    copyAttributesWithClip(labelYSP.style, sets.axisYLabelsStyle);
                }
                // 禁止事件
    ...
    /**
     * @function  Zondy.Feature.ShapeParameters.prototype.ShapeStyleTool
     * @description 一个图形 style 处理工具。此工具将指定的默认 style，通用 style，按 styleGroup 取得的 style 和按数据值 value 范围取得的 style 进行合并，得到图形最终的 style。
     * @param {Object} defaultStyle - 默认style，此样式对象可设属性根据图形类型参考 <{@link Zondy.Feature.ShapeParameters}> 子类对象的 style 属性。
     * @param {Object} style - 图形对象基础 style，此参数控制图形的基础样式，可设属性根据图形类型参考 <{@link Zondy.Feature.ShapeParameters}> 子类对象的 style 属性。优先级低于 styleGroup，styleByCodomain。
     * @param {Array.<Object>} styleGroup - 一个 style 数组，优先级低于 styleByCodomain，高于 style。此数组每个元素是样式对象，
     * 其可设属性根据图形类型参考 <{@link Zondy.Feature.ShapeParameters}> 子类对象的 style 属性。通过 index 参数从 styleGroup 中取 style。
    ...
    static ShapeStyleTool(defaultStyle, style, styleGroup, styleByCodomain, index, value) {
        ...
        // 基础 style
        if (style) {
            copyAttributesWithClip(finalStyle, style);
        }

        // 按索引赋 style
        if (styleGroup && styleGroup.length && typeof (index) !== "undefined" && !isNaN(index) && index >= 0) {
            if (styleGroup[index]) {
                copyAttributesWithClip(finalStyle, styleGroup[index]);
            }
        }
        ...
        if (styleByCodomain && styleByCodomain.length && typeof (value) !== "undefined") {
            ...
            for (var i = 0; i < dscLen; i++) {
                if (dsc[i].start <= v && v < dsc[i].end) {
                    copyAttributesWithClip(finalStyle, dsc[i].style);
                    break;
                }
            }
        }
...
export {ShapeFactory};
Zondy.Feature.ShapeFactory = ShapeFactory;