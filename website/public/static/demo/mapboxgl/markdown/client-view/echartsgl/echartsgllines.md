...
&ensp;&ensp;&ensp;&ensp;本实例通过读取json文件的世界航线数据，在地图中可视化的展示不同城市之间的航班飞行轨迹。

### 示例实现

&ensp;&ensp;&ensp;&ensp;本示例需要使用【include-mapboxgl-local.js】开发库实现，通过echarts框架的关键接口`setOption()`实现世界航线加载，示例通过<a target="_blank" href="https://echarts.apache.org/zh/tutorial.html#%E4%BD%BF%E7%94%A8%20ECharts%20GL%20%E5%AE%9E%E7%8E%B0%E5%9F%BA%E7%A1%80%E7%9A%84%E4%B8%89%E7%BB%B4%E5%8F%AF%E8%A7%86%E5%8C%96">Echarts GL</a>进行数据的渲染显示。
...
**Step 2. <font color=red>获取世界航线数据</font>**：
 &ensp;&ensp;&ensp;&ensp;通过`$.getJSON()`接口获取到世界航线数据，将拿到的数据处理为echarts需要的格式；

* Example:
   ```javascript
      $.getJSON("../../static/data/echartsgl/lines/lines.json",  function(){})
   ```
**Step 3. <font color=red>页面展示世界航线可视化效果</font>**：
 &ensp;&ensp;&ensp;&ensp;通过echarts的`setOption()`方法加载地图与世界航线数据进行可视化显示。