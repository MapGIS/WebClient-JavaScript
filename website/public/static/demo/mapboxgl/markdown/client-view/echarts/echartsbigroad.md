...
在中国范围内的地图上加载100万道路线数据，展示大数据量的地图可视化效果。

### 示例实现

本示例需要使用include-mapboxgl-local.js开发库实现，通过关键接口`mapboxgl.zondy.EchartsLayer()`实现中国100万道路线数据的加载。
...
4. 加载中国-100万线数据，通过mapboxgl扩展的EchartsLayer图层实现，其中的参数option与<a target="_blank" href="http://echarts.baidu.com/api.html#echarts">echarts</a>方法`setOption`中一致；
