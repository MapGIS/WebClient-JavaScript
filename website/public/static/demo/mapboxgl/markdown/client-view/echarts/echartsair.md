...
PM2.5全国主要城市空气质量，通过可视化直观查看全国不同城市空气质量的差异。

### 示例实现

本示例需要使用include-mapboxgl-local.js开发库实现，通过关键接口`mapboxgl.zondy.EchartsLayer()`实现空气质量数据的加载。
...
4. 加载空气质量数据，通过mapboxgl扩展的EchartsLayer图层实现，其中的参数option与<a target="_blank" href="http://echarts.baidu.com/api.html#echarts">echarts</a>方法`setOption`中一致；
