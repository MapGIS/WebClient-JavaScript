...
用点的密集程度来表示与范围或区域面积相关联数据值，适用于表示具有数量特征散分布的专题。

### 示例实现

本示例需要使用include-mapboxgl-local.js开发库实现，通过关键接口`Zondy.Service.ThemeOper()`实现MapGIS地图文档的`密度专题图`操作功能。
...
4. 创建**专题图结构信息对象**，并将该对象存放数组中，执行专题图功能时使用，如`添加专题图`、`更新专题图`、`删除专题图`。重点内容：使用`Zondy.Object.Theme.CSimpleTheme()`实现**密度**专题图；
...
#### 2.【点密度专题图图形信息类】CDotDensityTheme

##### `Zondy.Object.Theme.CDotDensityTheme(opt_options)`：点密度专题图图形信息类的构造函数

> `CDotDensityTheme`主要参数
